...
perpPlaneField=zeros(length(rawPoints)/7,1); %magnitude of field components in plane perpendicular to optical axis (y-z)

n=1;
k=1;
perpPlaneFields=zeros(length(rawPoints)/7,1);
while k<=(length(rawPoints)/7)
    ...
    perpPlaneField(k)=sqrt(abs(fieldsVsFreq(k,3))^2+abs(fieldsVsFreq(k,4))^2);
